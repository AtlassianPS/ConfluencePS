...
Describe 'Integration Tests' -Tag Integration, Cloud, DataCenter {
    ...
    Context 'Invoke-ConfluenceMethod' {
        BeforeAll {
            ...
            $script:InvokeMethodCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList ($script:integrationEnvironment.Username, $script:InvokeMethodSecurePassword)
        ...
        It 'returns at least one space result from a direct API call' {
            @($InvokeMethodSpaceResults).Count | Should -BeGreaterThan 0
            @($InvokeMethodSpaceResults)[0].ID | Should -Not -BeNullOrEmpty
            @($InvokeMethodSpaceResults)[0].Key | Should -Not -BeNullOrEmpty
        }

        It 'can cast direct API results to ConfluencePS.Space' {
            @($InvokeMethodTypedSpaceResults).Count | Should -BeGreaterThan 0
            @($InvokeMethodTypedSpaceResults)[0] | Should -BeOfType [ConfluencePS.Space]
            @($InvokeMethodTypedSpaceResults)[0].Key | Should -Not -BeNullOrEmpty
        }
    }
